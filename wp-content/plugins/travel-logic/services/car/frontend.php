<?php
/*
 * Travel Logic - car service front end functionality.
 */
...
/**
 * This is a secret field name for identifying car search requests
 */
define( 'TLOGIC_CAR_SEARCH_KEY', '7524d0d4c61a' );
 ...
 * @return boolean
 */
function tlogic_frontend_is_car_search()
{
	// get search key field value
	$search_key = tlogic_get_field_value( TLOGIC_CAR_SEARCH_KEY );

	// if not not, its a search request
	return ( null !== $search_key );
}

function tlogic_frontend_do_car_search()
{
	if ( false === tlogic_frontend_is_car_search() ) {
		// not a search request...
	} else {
		return tlogic_frontend_do_car_results();
	}
}

function tlogic_frontend_do_car_results()
{
	// parse request
	$data = tlogic_frontend_parse_car_search();

    // validate request
    if ( true === tlogic_frontend_validate_car_search( $data ) ) {

        // new car search instance
        $search = new Travel_Logic_Car_Search();
		...
		$args = array(
            ...
            'Occupancy'   => array(
                ...
                'ChildAges'  => tlogic_frontend_car_child_age(),
            )
		);
        ...
        add_action( 'wp_enqueue_scripts',
            'tlogic_frontend_enqueue_car_script' );
	}
}

function tlogic_frontend_enqueue_car_script()
{
    $handle = 'tlogic_result';

    // CONTROLLER
    wp_enqueue_script( $handle, TLOGIC_ASSETS_URL . '/tlogic-car-result.js',
        array( 'angular', 'angular-animate', 'underscore' ), '20150218_01',
        FALSE );

    $tlogic_car = tlogic_cars_template()->toArray();

    $tlogic_car['config'] = array_merge( $tlogic_car['config'], array(
        'TLOGIC_ASSETS_URL' => TLOGIC_ASSETS_URL
    ) );

    wp_localize_script( $handle, $handle, $tlogic_car );
}

function tlogic_frontend_do_car_details()
{
	// new car details instance
	$details = new Travel_Logic_Car_Valuation();
	...
	// set up template instance
	tlogic_cars_template( $details );

}

function tlogic_frontend_do_book_car()
{
    ...
    elseif ( isset( $_POST[ 'tlogic_customer_book' ] ) ) {
        ...
        // new car details instance
        $confirmation = new Travel_Logic_Car_Confirm_Booking();
        ...
        // set up template instance
        tlogic_cars_template( $confirmation );
        // die( 'do not know what to do...' );
    }
    else {

        // new car details instance
        $details = new Travel_Logic_Car_Book_Car();
...
/**
 * Return array of valid and normalized fields from car search request
 *
 * @return array
 */
function tlogic_frontend_parse_car_search()
{
 ...
 * @return boolean
 */
function tlogic_frontend_validate_car_search( $d )
{
	// error templates
	$tpls = array(
...
function tlogic_frontend_car_child_age()
{
